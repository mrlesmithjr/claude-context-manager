...
**Status**: ACTIVE
**Last Updated**: June 3, 2026 (v0.8.159)
...
| 30 | Branch-aware capture | Opt-in via `CONTEXT_MANAGER_BRANCH_AWARE` (default off). When enabled: `getCurrentBranch()` via `spawnSync`; branch stored on observations and sessions; soft-rank boost in search; filter on tag path. When off, `getCurrentBranch()` is bypassed on every hook and `branch` is stored as null. |
...
| 31 | Fact supersession | `FACT_CATEGORIES` in `facts.ts`; `superseded_by` column; `findConflictingFact()` marks old fact superseded on save; excluded from search by default; `include_superseded` param to opt in. `countObservations()` also excludes superseded rows by default (`includeSuperseded?: boolean` param to opt in), matching `search()` behavior to prevent pagination over-count. |
...
| 47 | Auto-write skill lessons at Stop | `getSessionLessonCandidates()` fetches observations where `skill IS NOT NULL`, `lesson_type IS NOT NULL`, or `importance_score >= 0.65`; `writeSessionLessons()` groups by skill name, threshold: invocation importance >= 0.5 OR `lesson_type` present; appends dated bullet entries to `~/.claude/skills/<name>/.lessons.md`; agent groups are silently skipped (agents use native Claude Code memory); local mode only; rule-based, no LLM calls |
...
| 50 | FK constraint recovery on capture | `POST /capture/observation` catches FOREIGN KEY constraint errors from `storage.save()`; on FK error, calls `storage.createSession(sessionId, normalizedProject, branch ?? null)` to create a minimal session row, then retries the save once; any non-FK error is re-thrown immediately. Two triggers: (1) sub-agent sessions where `SessionStart` hook never fired, leaving `session_id` absent from `sessions`; (2) regular sessions where `remoteCreateSession()` failed at startup due to a network error or server restart during the hook. (`src/server/http.ts`) |
| 51 | Agent PostToolUse `cwd` fallback in remote mode | Claude Code omits `cwd` from Agent tool PostToolUse events. Remote mode in `capture-tool.ts` now falls back to `process.cwd()` when `obj.cwd` is absent (`typeof obj.cwd !== 'string'`), matching the existing `process.cwd()` fallback in `validatePostToolUseInput()` for local mode. Without this, `rawCwd` was `''`, triggering the early-exit skip guard before any HTTP call was made, silently dropping all Agent PostToolUse captures in remote mode. (`plugin/hooks/capture-tool.ts`) |
| 52 | context_mine backfill | `context_mine` MCP tool walks `~/.claude/projects/` JSONL transcripts, pairs `tool_use` + `tool_result` events by `tool_use_id` using a single interleaved pass, runs through the existing `processToolCapture()` pipeline, skips sessions already in DB, inserts sessions as `source='mine'`/`status='complete'`, local mode only (returns error if `CONTEXT_MANAGER_URL` is set). `decodeDashedPath()` uses filesystem-validated greedy decode with path traversal guard (`./..`). Accepts `project` (limit to one path), `dry_run` (default false), `limit_sessions` (cap for incremental runs). Files: `src/capture/mine.ts`, `src/utils/transcript.ts` |
...
| 38 | Remote parity | `remoteCreateSession` forwards branch; `GET /api/decisions/next-number` for globally sequential decision numbering in remote mode; `POST /capture/observation` forwards `lesson_type`, `skill`, `branch`, and `package` so remote captures have full field parity with local captures; remote mode `capture-tool.ts` falls back to `process.cwd()` when `cwd` is absent (Agent events), matching local mode behavior (see #51) |
...
| `CONTEXT_MANAGER_BRANCH_AWARE` | _(unset / off)_ | Enables git branch capture (Decision 30) and the soft-rank boost. Default off. Truthy: `1`, `true`, `yes` (case-insensitive). When off, `getCurrentBranch()` is bypassed on every hook and `branch` is stored as null. |
