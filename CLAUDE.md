...
### 20. Stale Session GC in context_vacuum (v0.8.37) + auto-GC on SessionStart (v0.8.38)
...
**What it does (v0.8.37):** `vacuum()` in `src/storage/sqlite.ts` marks `active` sessions as `complete` when their last activity timestamp is older than N hours (default: 2). The activity timestamp is `last_checkpoint_at` when available (written every 30 minutes by the periodic checkpoint; see Design Decision #18). For sessions that predate the checkpoint column or where no checkpoint has run, it falls back to `started_at`.
...
**Schema effect:** The GC call sets `ended_at = datetime('now')` and `status = 'complete'` on matching rows. No observations are deleted by this step; only the session status is updated.

**v0.8.37 surface area:**
...
**v0.8.38: automatic GC on SessionStart:** The stale session logic was extracted from `vacuum()` into a standalone method `closeStaleActiveSessions(staleSessionHours = 2)` on `StorageInterface` and `SQLiteStorage`. `vacuum()` now delegates to this method (no SQL duplication). `context-inject.ts` (SessionStart hook) calls `closeStaleActiveSessions()` silently after creating the new session in local mode only — remote mode is unaffected. GC failure never blocks session start (wrapped in a silent try/catch). Result: stuck sessions are cleaned up automatically on every session open. Users no longer need to invoke `context_vacuum` manually to close stale sessions; `context_vacuum` remains the right tool for observation compaction and full database optimization.
...
|------|---------|---------|---------|
| `SessionStart` | Create session, inject status hint, run stale session GC (local mode) | 10s | `startup\|clear\|compact` |
