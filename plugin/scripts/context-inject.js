...
function checkPostUpdate() {
  try {
    ...
    if (stored === "0.8.171") return "";
    const verb = stored === "" ? "Installed" : "Updated";
    return `[context-manager] ${verb} v${"0.8.171"}. Hooks active.`;
  } catch {
    return "";
  }
}
function markVersionActivated() {
  try {
    ...
    writeFileSync(PLUGIN_VERSION_FILE, "0.8.171", "utf-8");
  } catch {
  }
}
...
async function main() {
  ...
  try {
    ...
    if (remoteUrl) {
      ...
      lines2.push(`context-manager v${"0.8.171"} active (remote mode). ${remoteCount} observations on server.`);
    ...
    lines.push(`context-manager v${"0.8.171"} active. ${count} observations tracked.${branchHint}`);
